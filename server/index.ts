...
(async () => {
  ...
  server.listen({
    ...
    // CSV import has been moved to manual endpoints only to prevent deployment crash loops
    // Data can be imported via POST /api/pl/import-csv and POST /api/cashflow/import-csv
    log(`[startup] Server ready. CSV import available via API endpoints.`);
  });
})();
